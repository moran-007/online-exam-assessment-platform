    ...
    <div v-if="canManageOwnExternalAccounts" class="panel profile-panel hydro-account-panel">
      ...
      <el-form label-width="112px" class="hydro-account-form">
        ...
        <el-form-item label="OJ站点">
          <el-input v-model="hydroForm.platformBaseUrl" placeholder="http://moran007.top" />
        </el-form-item>
...
function resetHydroForm() {
  ...
  hydroForm.platformBaseUrl = platforms.value[0]?.baseUrl || 'http://moran007.top';
...
function defaultPlatformBaseUrl() {
  return platforms.value.find((item) => item.code === hydroForm.platformCode)?.baseUrl || 'http://moran007.top';
}
...
function openHydroUser() {
  ...
  const baseUrl = hydroForm.platformBaseUrl.replace(/\/+$/, '') || 'http://moran007.top';
