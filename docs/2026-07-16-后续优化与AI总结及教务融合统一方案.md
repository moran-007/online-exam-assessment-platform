...
- 是否支持流式输出；
- 上下文窗口和最大输出；
- thinking 模式与结构化输出是否兼容；当前结构化非流式请求对 DeepSeek 和 Qwen/阿里云兼容端点均显式关闭思考。
...
- 考试总结 800–1200、学生总结 500–800 仅作为成本估算参考，不是隐藏默认值或必填限制；1,000 Token 也只作参考。
- 配置级和调用级上限均可为空；两者都为空时不发送 `max_tokens`。存在显式值时取调用级、配置级和模型能力上限的最小值，8,192/能力上界只承担输入校验和 usage 未报告时的保守预留。
