...
区分供应商超时和本地任务租约超时。调大配置的 `timeoutMs` 前先做最小连接；不要用增加输出 Token 掩盖网络问题。供应商未报告准确用量时，系统按本次请求上限保守预留，后续不会伪造实际 Token。

### Token 上限不一致

“本次输出上限”留空表示不增加调用级限制，使用模型配置上限；填写数字时取调用级、模型能力和配置上限中的最小值。界面显示“自动”时，请求不能偷偷发送历史默认值。用量以 `ai_usage_events` 为审计来源。
