...
- 阶段 1 最初以约 1,000 输出 Token 估算真实验收成本；后续统一修正为成本参考，不作为系统默认值或必填限制。
- 配置级 `maxTokens` 和调用级 `requestOutputTokens` 都可为空；两者都为空时不向供应商发送 `max_tokens`。存在显式值时取调用级、配置级和已登记模型能力上限的最小值。
- 8,192 平台校验上界和模型能力上界用于校验显式输入，并在供应商未报告 usage 时确定保守预算预留，不是隐藏请求限制。
- 模型返回的 prompt/completion/total Token 写入追加式 `ai_usage_events` 流水，业务代码不提供修改入口。
- 若兼容接口未返回 usage，则记录为“未报告调用”，按解析后的预留上界保守占用本地预算，不会把未知用量伪装成 0 消耗；仍有可计算余额时可以继续调用。
...
- 阶段 1 当时计划在阶段 2 闭环后显式使用约 1,000 输出 Token 做最小受控验收，并记录实际使用与剩余本地预算；该数值是该次验收参数和成本参考，不代表后续默认值。
