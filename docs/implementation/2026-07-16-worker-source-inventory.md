...
该结论仅说明“本机最完整开发副本”。`class_worker` 文档明确会初始化 30 名模拟学员，`worker_01` 又完整继承这些数据，因此当前 SQLite 不能被认定为真实生产副本。生产迁移前仍必须取得只读快照、业务签字和独立附件清单。

## 2. 候选目录取舍

| 目录 | 技术/状态 | 本地数据 | 处置 |
| --- | --- | --- | --- |
| `worker_01` | Flask + Vue + SQLite/MySQL 开关；Scratch P2；Git 干净 | 25 张业务表，数据库 491,520 bytes，更新至 2026-06-10 | 迁移开发唯一首选源；生产前换成真实只读快照 |
...
本地代码版本、Schema、枚举、行数、血缘和附件完整性已盘点，可以用于 migration dry-run 工具开发。以下条件仍未满足，不能进入生产导入或课时切换：

- 真实生产数据库只读副本及其 SHA-256；
- 真实生产附件快照和逐文件清单；
- 身份冲突清单业务签字；
- 家长拆分规则和课时开账边界签字；
- 旧 Secret 轮换与历史保留周期确认。
